...
若本次脚手架涉及新增或建议 `package.json` 依赖，遵循工作区 `skills/frontend-craft-rules/react.md` 或 `skills/frontend-craft-rules/vue.md` 中「版本与依赖」约定；尚无对应规则文件时，优先采用彼此兼容的主流稳定版本（对齐官方文档、脚手架默认值或包管理器推荐）。
...
```
用法：`scaffold` 命令 + `<type> <Name>` 参数（具体以当前客户端 slash 命令前缀为准）
示例：
  scaffold page UserDetail
  scaffold feature payment
  scaffold component DataTable
```
