...
4. 完成后根据项目技术栈编辑 `AGENTS.md` 中的命令与栈说明，并删除 `skills/frontend-craft-rules/` 下不适用的规则文件；填写或更新 `package.json`、文档化依赖版本时，遵循 `skills/frontend-craft-rules/react.md` 或 `skills/frontend-craft-rules/vue.md` 中「版本与依赖」条目（彼此兼容的主流稳定版本，对齐官方文档、脚手架默认值或包管理器推荐）。
